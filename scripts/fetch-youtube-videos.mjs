...
const videoIds = fs.readFileSync(videoidsPath, 'utf-8')
  .split('\n')
  .map(line => line.trim())
...
async function fetchVideoMetadata(videoId) {
  ...
  try {
    ...
    let description = '';
    let publishDate = '';
    let thumbnail = oEmbedData?.thumbnail_url || `https://img.youtube.com/vi/${videoId}/maxresdefault.jpg`;
    ...
    for (const match of jsonLdMatches) {
      try {
        ...
        if (videoData) {
          ...
          publishDate = videoData.uploadDate || videoData.datePublished || publishDate;
        ...
        try {
          ...
          if (videoDetails) {
            ...
            if (videoDetails.thumbnail?.thumbnails?.length > 0) {
              thumbnail = videoDetails.thumbnail.thumbnails[videoDetails.thumbnail.thumbnails.length - 1].url || thumbnail;
            }
          }
        } catch (e) {
    ...
    if (!title) {
      ...
      title = titleMatch ? titleMatch[1].replace(/\s*-\s*YouTube$/, '').trim() : `Video ${videoId}`;
    }
