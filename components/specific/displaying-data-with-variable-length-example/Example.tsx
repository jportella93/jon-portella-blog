...
const user2 = new User(
  "Antonio José",
  "de los Santos",
  "antonio_jose_santos108@outlook.com"
);
...
const Example = ({ title, description, withScrollers }: ExampleProps) => {
  ...
  return (
    <>
      <h2 id={title.split(" ").join("-").toLowerCase()}>{title}</h2>
      <p style={{ maxWidth: "500px" }}>{description}</p>
      <div style={holderStyle}>
        <div style={btnsWrapperStyle}>
          <button style={btnStyle} onClick={() => setUser(user1)}>
            Show user 1
          </button>
          <button style={btnStyle} onClick={() => setUser(user2)}>
            Show user 2
          </button>
        </div>
