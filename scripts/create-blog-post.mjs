...
async function promptMultiline() {
  ...
  console.log('Paste your blog post (first line = title, second line = spoiler, rest = content):');
...
async function createBlogPost() {
  try {
    ...
    if (lines.length < 2) {
      console.error('Error: Content must have at least 2 lines (title and spoiler)');
      process.exit(1);
    }
...
`;
    ...
    console.log(`  Images directory: ${path.relative(process.cwd(), imagesDirectory)}`);
