...
const existingSlugs = entries
  .filter(e => e.isDirectory())
  .map(e => e.name);

// Get numbered posts
const numberedPosts = entries
  .filter(e => e.isDirectory() && e.name.startsWith('1-minute-to-become-a-better-developer-'))
  .map(e => {
    const indexPath = path.join(postsDir, e.name, 'index.md');
    const content = fs.readFileSync(indexPath, 'utf8');
    const { data } = matter(content);
    let newSlug = data.title.toLowerCase()
      .replace(/[^a-z0-9]+/g, '-')
      .replace(/^-+|-+$/g, '');
    ...
    if (existingSlugs.includes(newSlug) && e.name !== newSlug) {
      ...
      if (existingData.title === data.title && existingData.date === data.date) {
